---
description: Full pre-deployment pipeline for Terraform - security, quality, validate, plan, commit, PR creation, CI/CD monitoring, and post-merge cleanup.
---
...
# SHIP COMMAND - Terraform Pre-Deployment Pipeline

You are orchestrating the complete ship process for a Terraform project. This is a sequential pipeline where each phase must complete successfully before proceeding.
...
## PHASE 2: Validation and Plan

Run Terraform validation and generate a plan:
```bash
terraform validate
terraform plan -out=tfplan
```
- Review the plan output for unexpected changes
- If destructive changes are detected, STOP and ask the user for confirmation

**GATE CHECK**: Plan must generate without errors. Destructive changes require user approval.
...
3. Create a professional Pull Request description including the plan summary
...
- Run all phases continuously without stopping for confirmation (EXCEPT destructive plan changes)
